...
package com.google.android.globaltime;
 ...
 * 
 * <p>
 * A GLView object stores common elements of GL state and allows them to be
 * modified interactively. This is particularly useful for determining the
 * proper settings of parameters such as the view frustum and light intensities
 * during application development.
 * 
 * <p>
 * A GLView is not an actual View; instead, it is meant to be called from within
 * a View to perform event processing on behalf of the actual View.
 * 
 * <p>
 * By passing key events to the GLView object from the View, the application can
 * automatically allow certain parameters to be user-controlled from the
 * keyboard. Key events may be passed as shown below:
 * 
 * <pre>
 ...
 * 
 * <p>
 * During drawing of a frame, the GLView object should be given the opportunity
 * to manage GL parameters as shown below:
 * 
 * OpenGLContext mGLContext; // initialization not shown int mNumTrianglesDrawn =
 * 0;
 * 
 * protected void onDraw(Canvas canvas) { int w = getWidth(); int h =
 * getHeight();
 * 
 * float ratio = (float) w / h; mGLView.setAspectRatio(ratio);
 * 
 * GL10 gl = (GL10) mGLContext.getGL(); mGLContext.waitNative(canvas, this);
 *  // Enable a light for the GLView to manipulate
 * gl.glEnable(GL10.GL_LIGHTING); gl.glEnable(GL10.GL_LIGHT0);
 *  // Allow the GLView to set GL parameters mGLView.setTextureParameters(gl);
 * mGLView.setProjection(gl); mGLView.setView(gl); mGLView.setLights(gl,
 * GL10.GL_LIGHT0);
 *  // Draw some stuff (not shown) mNumTrianglesDrawn += <num triangles just
 * drawn>;
 *  // Wait for GL drawing to complete mGLContext.waitGL();
 *  // Inform the GLView of what was drawn, and ask it to display statistics
 * mGLView.setNumTriangles(mNumTrianglesDrawn); mGLView.showMessages(canvas);
 * mGLView.showStatistics(canvas, w); }
 * 
 * </pre>
 * 
 * <p>
 * At the end of each frame, following the call to GLContext.waitGL, the
 * showStatistics and showMessages methods will cause additional information to
 * be displayed.
 * 
 * <p>
 * To enter the interactive command mode, the 'tab' key must be pressed twice in
 * succession. A subsequent press of the 'tab' key exits the interactive command
 * mode. Entering a multi-letter code sets the parameter to be modified. The
 * 'newline' key erases the current code, and the 'del' key deletes the last
 * letter of the code. The parameter value may be modified by pressing the
 * keypad left or up to decrement the value and right or down to increment the
 * value. The current value will be displayed as an overlay above the GL
 * rendered content.
 * 
 * <p>
 * The supported keyboard commands are as follows:
 * 
 * <ul>
...
public class GLView {
    ...
    private static final String[] commands = { "fn", "ff", "tx", "ty", "tz",
            "z", "la", "lar", "lag", "lab", "ld", "ldr", "ldg", "ldb", "ls",
            "lsr", "lsg", "lsb", "lma", "lmar", "lmag", "lmab", "tmin", "tmag",
            "tper" };

    private static final String[] labels = { "Near Frustum", "Far Frustum",
            "Translate X", "Translate Y", "Translate Z", "Zoom",
            "Ambient Intensity", "Ambient Red", "Ambient Green",
            "Ambient Blue", "Diffuse Intensity", "Diffuse Red",
            "Diffuse Green", "Diffuse Blue", "Specular Intenstity",
            "Specular Red", "Specular Green", "Specular Blue",
            "Light Model Ambient Intensity", "Light Model Ambient Red",
            "Light Model Ambient Green", "Light Model Ambient Blue",
            "Texture Min Filter", "Texture Mag Filter",
            "Texture Perspective Correction", };

    private static final float[] defaults = { 5.0f, 100.0f, 0.0f, 0.0f, -50.0f,
            0, 0.125f, 1.0f, 1.0f, 1.0f, 0.125f, 1.0f, 1.0f, 1.0f, 0.125f,
            1.0f, 1.0f, 1.0f, 0.125f, 1.0f, 1.0f, 1.0f, GL10.GL_NEAREST,
            GL10.GL_NEAREST, GL10.GL_FASTEST };

    private static final float[] increments = { 0.01f, 0.5f, 0.125f, 0.125f,
            0.125f, 1.0f, 0.03125f, 0.1f, 0.1f, 0.1f, 0.03125f, 0.1f, 0.1f,
            0.1f, 0.03125f, 0.1f, 0.1f, 0.1f, 0.03125f, 0.1f, 0.1f, 0.1f, 0, 0,
            0 };
    ...
    /**
     * Sets the overall ambient light intensity. This intensity will be used to
     * modify the ambient light value for each of the red, green, and blue
     * channels passed to glLightfv(...GL_AMBIENT...). The default value is
     * 0.125f.
     * 
     * @param intensity
     *            a floating-point value controlling the overall ambient light
     *            intensity.
     */
    ...
    /**
     * Sets the light model ambient intensity. This intensity will be used to
     * modify the ambient light value for each of the red, green, and blue
     * channels passed to glLightModelfv(GL_LIGHT_MODEL_AMBIENT...). The default
     * value is 0.125f.
     * 
     * @param intensity
     *            a floating-point value controlling the overall light model
     *            ambient intensity.
     */
    ...
    /**
     * Sets the ambient color for the red, green, and blue channels that will be
     * multiplied by the value of setAmbientIntensity and passed to
     * glLightfv(...GL_AMBIENT...). The default values are {1, 1, 1}.
     * 
     * @param ambient
     *            an arry of three floats containing ambient red, green, and
    ...
    /**
     * Sets the overall diffuse light intensity. This intensity will be used to
     * modify the diffuse light value for each of the red, green, and blue
     * channels passed to glLightfv(...GL_DIFFUSE...). The default value is
     * 0.125f.
     * 
     * @param intensity
     *            a floating-point value controlling the overall ambient light
    ...
    /**
     * Sets the diffuse color for the red, green, and blue channels that will be
     * multiplied by the value of setDiffuseIntensity and passed to
     * glLightfv(...GL_DIFFUSE...). The default values are {1, 1, 1}.
     * 
     * @param diffuse
     *            an array of three floats containing diffuse red, green, and
     *            blue intensity values.
     */
    ...
    /**
     * Sets the overall specular light intensity. This intensity will be used to
     * modify the diffuse light value for each of the red, green, and blue
     * channels passed to glLightfv(...GL_SPECULAR...). The default value is
     * 0.125f.
     * 
     * @param intensity
     *            a floating-point value controlling the overall ambient light
    ...
    /**
     * Sets the specular color for the red, green, and blue channels that will
     * be multiplied by the value of setSpecularIntensity and passed to
     * glLightfv(...GL_SPECULAR...). The default values are {1, 1, 1}.
     * 
     * @param specular
     *            an array of three floats containing specular red, green, and
     *            blue intensity values.
     */
    ...
    /**
     * Returns the current X translation of the modelview transformation as
     * passed to glTranslatef. The default value is 0.0f.
     * 
    ...
    /**
     * Returns the current Y translation of the modelview transformation as
     * passed to glTranslatef. The default value is 0.0f.
     * 
    ...
    /**
     * Returns the current Z translation of the modelview transformation as
     * passed to glTranslatef. The default value is -50.0f.
     * 
    ...
    /**
     * Sets the position of the near frustum clipping plane as passed to
     * glFrustumf. The default value is 5.0f;
     * 
     * @param nearFrustum
     *            the near frustum clipping plane distance as a float.
     */
    ...
    /**
     * Sets the position of the far frustum clipping plane as passed to
     * glFrustumf. The default value is 100.0f;
     * 
     * @param farFrustum
     *            the far frustum clipping plane distance as a float.
     */
    ...
    private void computeZoom() {
        mZoom = mZoomScale
                * (float) Math.pow(mZoomBase, -params[ZOOM_EXPONENT]);
    }
    ...
    /**
     * Displays the message overlay on the given Canvas. The GLContext.waitGL
     * method should be called prior to calling this method. The interactive
     * command display is drawn by this method.
     * 
     * @param canvas
     *            the Canvas on which messages are to appear.
     */
    ...
    /**
     * Sets the number of triangles drawn in the previous frame for display by
     * the showStatistics method. The number of triangles is not computed by
     * GLView but must be supplied by the calling Activity.
     * 
     * @param triangles
     *            an Activity-supplied estimate of the number of triangles drawn
     ...
     * GLContext.waitGL method should be called prior to calling this method.
     * 
     * @param canvas
     *            the Canvas on which statistics are to appear.
     * @param width
    ...
    public void showStatistics(Canvas canvas, int width) {
        ...
        if (mDisplayCounts) {
            canvas.drawText(mTriangles + " triangles", width - 100.0f,
                    10.0f + th + 5, mPaint);
        }
    }
    ...
    private void toggleFilter() {
        ...
        addMessage(commands[mParam], "Texture "
                + (mParam == TEXTURE_MIN_FILTER ? "min" : "mag") + " filter = "
                + (params[mParam] == GL10.GL_NEAREST ? "nearest" : "linear"));
    }
    ...
    private void togglePerspectiveCorrection() {
        ...
        addMessage(commands[mParam], "Texture perspective correction = "
                + (params[mParam] == GL10.GL_FASTEST ? "fastest" : "nicest"));
    }

    private String valueString() {
        if (mParam == TEXTURE_MIN_FILTER || mParam == TEXTURE_MAG_FILTER) {
            if (params[mParam] == GL10.GL_NEAREST) {
                return "nearest";
            }
    ...
    /**
     * Process a key stroke. The calling Activity should pass all keys from its
     * onKeyDown method to this method. If the key is part of a GLView command,
     * true is returned and the calling Activity should ignore the key event.
     * Otherwise, false is returned and the calling Activity may process the key
     * event normally.
     * 
     * @param keyCode
     *            the key code as passed to Activity.onKeyDown.
     * 
     * @return true if the key is part of a GLView command sequence, false
     *         otherwise.
     */
    public boolean processKey(int keyCode) {
        // Pressing the state key twice enters the UI
        // Pressing it again exits the UI
        if ((keyCode == STATE_KEY) || (keyCode == KeyEvent.KEYCODE_SLASH)) {
            mState = (mState + 1) % 3;
            if (mState == HAVE_NONE) {
                clearMessages();
            }
        ...
        if (addMessage) {
            addMessage(commands[mParam], labels[mParam] + ": " + valueString());
        }
    ...
    /**
     * Zoom in by a given number of steps. A negative value of steps zooms out.
     * Each step zooms in by 1%.
     * 
     * @param steps
     *            the number of steps to zoom by.
     */
    ...
    /**
     * Set the projection matrix using glFrustumf. The left and right clipping
     * planes are set at -+(aspectRatio*zoom), the bottom and top clipping
     * planes are set at -+zoom, and the near and far clipping planes are set to
     * the values set by setNearFrustum and setFarFrustum or interactively.
     * 
     * <p>
     * GL side effects:
     * <ul>
     ...
     * 
     * @param gl
     *            a GL10 instance whose projection matrix is to be modified.
     */
    public void setProjection(GL10 gl) {
        ...
        if (mAspectRatio >= 1.0f) {
            gl.glFrustumf(-mAspectRatio * mZoom, mAspectRatio * mZoom, -mZoom,
                    mZoom, params[NEAR_FRUSTUM], params[FAR_FRUSTUM]);
        } else {
            gl.glFrustumf(-mZoom, mZoom, -mZoom / mAspectRatio, mZoom
                    / mAspectRatio, params[NEAR_FRUSTUM], params[FAR_FRUSTUM]);
        }
    }

    /**
     * Set the modelview matrix using glLoadIdentity and glTranslatef. The
     * translation values are set interactively.
     * 
     * <p>
     * GL side effects:
     * <ul>
     ...
     * 
     * @param gl
     *            a GL10 instance whose modelview matrix is to be modified.
     */
    public void setView(GL10 gl) {
        ...
        gl.glTranslatef(params[TRANSLATE_X], params[TRANSLATE_Y],
                params[TRANSLATE_Z]);
    }
     ...
     * 
     * @param gl
     *            a GL10 instance whose texture parameters are to be modified.
     */
