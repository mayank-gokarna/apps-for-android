 ...
 * containing vertices, texture coordinates, colors, normals, and indices. The
 * {@link #draw(GL10)} method draws the object to the given OpenGL context.
 */
public abstract class Shape {
     ...
     * 
     * @param primitive
     *            a GL primitive type understood by glDrawElements, such as
     *            GL10.GL_TRIANGLES
     * @param indexDatatype
     *            the GL datatype for the index buffer, such as
     *            GL10.GL_UNSIGNED_SHORT
     * @param emitTextureCoordinates
     *            true to enable use of the texture coordinate buffer
     * @param emitNormals
     *            true to enable use of the normal buffer
     * @param emitColors
     *            true to enable use of the color buffer
     */
    protected Shape(int primitive, int indexDatatype,
            boolean emitTextureCoordinates, boolean emitNormals,
            boolean emitColors) {
    ...
    /**
     * Computes the cross-product of two vectors p and q and places the result
     * in out.
     */
    ...
    /**
     * Copies the given data into the instance variables mVertexBuffer,
     * mTexcoordBuffer, mNormalBuffer, mColorBuffer, and mIndexBuffer.
     * 
     * @param vertices
     *            an array of fixed-point vertex coordinates
     * @param texcoords
     *            an array of fixed-point texture coordinates
     * @param normals
     *            an array of fixed-point normal vector coordinates
     * @param colors
     *            an array of fixed-point color channel values
     * @param indices
     *            an array of short indices
     */
    public void allocateBuffers(int[] vertices, int[] texcoords, int[] normals,
        ...
        ByteBuffer ibb = ByteBuffer
                .allocateDirect(indices.length * SHORT_BYTES);
    ...
    public void allocateBuffers(int[] vertices, int[] texcoords, int[] normals,
        ...
        ByteBuffer ibb = ByteBuffer.allocateDirect(indices.length * INT_BYTES);
        ibb.order(ByteOrder.nativeOrder());
    ...
    private void allocate(int[] vertices, int[] texcoords, int[] normals,
            int[] colors) {
        ByteBuffer vbb = ByteBuffer.allocateDirect(vertices.length * INT_BYTES);
    ...
    public void draw(GL10 gl) {
        ...
        gl.glDrawElements(mPrimitive, mNumIndices > 0 ? mNumIndices
                : mIndexBuffer.capacity(), mIndexDatatype, mIndexBuffer);
    }
}
