...
public class Clock {
    ...
    /**
     * Adds a line to the given Path. The line extends from radius r0 to radius
     * r1 about the center point (cx, cy), at an angle given by pos.
     * 
     * @param path
     *            the Path to draw to
     * @param radius
     *            the radius of the outer rim of the clock
     * @param pos
     *            the angle, with 0 and 1 at 12:00
     * @param cx
     *            the X coordinate of the clock center
     * @param cy
     *            the Y coordinate of the clock center
     * @param r0
     *            the starting radius for the line
     * @param r1
     *            the ending radius for the line
     */
    private static void drawLine(Path path, float radius, float pos, float cx,
            float cy, float r0, float r1) {
     ...
     *            the Path to draw to
     */
    private static void drawVArrow(Path path, float cx, float cy, float width,
            float height) {
     ...
     *            the Path to draw to
     */
    private static void drawHArrow(Path path, float cx, float cy, float width,
            float height) {
    ...
    public void setCity(City city) {
        if (mCity != city) {
            if (mCity != null) {
                mOldOffset = (long) (mCity.getOffset() * (float) MILLISECONDS_PER_HOUR);
            } else if (city != null) {
                mOldOffset = (long) (city.getOffset() * (float) MILLISECONDS_PER_HOUR);
            } else {
     ...
     * @param alpha
     *            the translucency of the clock face
     * @param textAlpha
     *            the translucency of the text
     * @param showCityName
     *            if true, display the city name
     * @param showTime
     *            if true, display the time digitally
     * @param showUpArrow
     *            if true, display an up arrow
     * @param showDownArrow
     *            if true, display a down arrow
     * @param showLeftRightArrows
     *            if true, display left and right arrows
     * @param prefixChars
     *            number of characters of the city name to draw in bold
     */
    public void drawClock(Canvas canvas, float cx, float cy, float radius,
            float alpha, float textAlpha, boolean showCityName,
            boolean showTime, boolean showUpArrow, boolean showDownArrow,
            boolean showLeftRightArrows, int prefixChars) {
        ...
        float daylightOffset = tz.inDaylightTime(new Date(mTime)) ? tz.getDSTSavings() / (float) MILLISECONDS_PER_HOUR : 0.0f;
        ...
        String time = DateUtils.timeString(cal) + " "
                + DateUtils.getDayOfWeekStr(cal.get(Calendar.DAY_OF_WEEK))
                + " " + " (UTC" + (offset >= 0 ? "+" : "-") + offsetH
                + (offsetM == 0 ? "" : ":" + offsetM)
        ...
        // Set the text color
        paint.setARGB((int) (textAlpha * 255.0f), (int) (mColorRed * 255.0f),
                (int) (mColorGreen * 255.0f), (int) (mColorBlue * 255.0f));
        ...
        paint.setARGB((int) (alpha * 255.0f), (int) (mColorRed * 255.0f),
                (int) (mColorGreen * 255.0f), (int) (mColorBlue * 255.0f));
        ...
        canvas.drawOval(new RectF(cx - iradius, cy - iradius, cx + iradius, cy
                + iradius), paint);
        ...
        if (showUpArrow) {
            drawVArrow(path, cx + radius * 1.13f, cy - radius, radius * 0.15f,
                    -radius * 0.1f);
        }
        if (showDownArrow) {
            drawVArrow(path, cx + radius * 1.13f, cy + radius, radius * 0.15f,
