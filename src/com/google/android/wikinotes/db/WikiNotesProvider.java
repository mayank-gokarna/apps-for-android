...
/**
 * The Wikinotes Content Provider is the guts of the wikinotes application.
 * It handles Content URLs to list all wiki notes, retrieve a note by name,
 * or return a list of matching notes based on text in the body or the title
 * of the note. It also handles all set up of the database, and
 * reports back MIME types for the individual notes or lists of notes to
 * help Android route the data to activities that can display that data
 * (in this case either the WikiNotes activity itself, or the WikiNotesList
 * activity to list multiple notes).
 * 
 */
public class WikiNotesProvider extends ContentProvider {
    ...
    /**
     * This inner DatabaseHelper class defines methods to create and
     ...
     * URL. The URL can be either the all notes URL (in which case all
     * notes matching the where clause will be deleted), or the note
     * name, in which case it will delete the note with the exactly
     * matching title. Any of the other URLs will be rejected as
     * invalid.
     * 
     * For deleting notes by title we use ReST style arguments from the URI
     * and don't support where clause and args.
     */
    @Override
    ...
    /**
     * The update method, which will allow either a mass update using
     * a where clause, or an update targeted to a specific note name (the
     * latter will be more common). Other matched URLs will be
     * rejected as invalid.
     * 
     * For updating notes by title we use ReST style arguments from the URI
     * and do not support using the where clause or args.
     */
    @Override
